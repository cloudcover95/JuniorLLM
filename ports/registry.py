...
class LLMPort:
    name: str
    kind: str  # bitnet-native | high-quant | behavioral | edge-moe | agent-runtime
    quant: str
    backend: str  # mlx | bitnet.cpp | gguf | torch-ternary | none | astra-runner
...
PORTS = [
    ...
    LLMPort("JuniorAstra", "agent-runtime", "n/a", "astra-runner", 0.0, "Open Astra durable Work + ContextPipe; BYO BitNet LLM"),
...
def pick(task: str, ram_gb: float) -> LLMPort:
    t = (task or "").lower()
    if "astra" in t or "durable" in t or "checkpoint" in t or "long-horizon" in t:
        return next(p for p in PORTS if p.name == "JuniorAstra")
    if "field" in t or "beta" in t or "access" in t:
        return PORTS[0]
    if "safety" in t or "fable" in t:
        return next(p for p in PORTS if p.name == "JuniorFable")
    if ram_gb < 6:
        return PORTS[1] if ram_gb >= 2 else PORTS[0]
    if "chat" in t or "gemma" in t:
        return next(p for p in PORTS if p.name == "JuniorGemma4-4B")
    return PORTS[0]
