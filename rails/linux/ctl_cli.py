...
def run(argv: list[str], ns: dict) -> int:
    ...
    if cmd == "skill-pin":
        ...
        if sub == "last":
            dest = argv[3] if len(argv) > 3 else None
            report = ns["skill_pin_last"](dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "tail":
            token = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            if token is not None and dest is None and not token.strip().isdigit():
                dest = token
                token = None
            report = ns["skill_pin_tail"](token, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "head":
            token = argv[3] if len(argv) > 3 else None
            ...
            if token is not None and dest is None and not token.strip().isdigit():
                dest = token
                token = None
            report = ns["skill_pin_head"](token, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "count":
            dest = argv[3] if len(argv) > 3 else None
            report = ns["skill_pin_count"](dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "genesis":
            dest = argv[3] if len(argv) > 3 else None
            report = ns["skill_pin_genesis"](dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "parent":
            hdr = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            report = ns["skill_pin_parent"](hdr, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "child":
            hdr = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            report = ns["skill_pin_child"](hdr, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "children":
            hdr = argv[3] if len(argv) > 3 else None
        ...
        if sub == "siblings":
            hdr = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            report = ns["skill_pin_siblings"](hdr, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "cousins":
            hdr = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            report = ns["skill_pin_cousins"](hdr, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "uncles":
            hdr = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            report = ns["skill_pin_uncles"](hdr, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "nephews":
            hdr = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            report = ns["skill_pin_nephews"](hdr, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "grandchildren":
            hdr = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            report = ns["skill_pin_grandchildren"](hdr, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "great-grandchildren":
            hdr = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            report = ns["skill_pin_great_grandchildren"](hdr, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        if sub == "great-great-grandchildren":
            hdr = argv[3] if len(argv) > 3 else None
            dest = argv[4] if len(argv) > 4 else None
            report = ns["skill_pin_great_great_grandchildren"](hdr, dest)
            print(json.dumps(report, indent=2, default=str))
            return 0 if report["ok"] else 1
        print(
            "usage: juniorctl skill-pin list [ROOT] | skill-pin load REL [ROOT] | skill-pin pin REL [ROOT] | skill-pin verify [ROOT] | skill-pin verify-one REL [ROOT] | skill-pin tip [ROOT] | skill-pin log [ROOT] | skill-pin height [ROOT] | skill-pin get HEIGHT [ROOT] | skill-pin at HDR [ROOT] | skill-pin range FROM TO [ROOT] | skill-pin since HDR [ROOT] | skill-pin until HDR [ROOT] | skill-pin before HDR [ROOT] | skill-pin after HDR [ROOT] | skill-pin first [ROOT] | skill-pin last [ROOT] | skill-pin tail [N] [ROOT] | skill-pin head [N] [ROOT] | skill-pin count [ROOT] | skill-pin genesis [ROOT] | skill-pin parent HDR [ROOT] | skill-pin child HDR [ROOT] | skill-pin children HDR [ROOT] | skill-pin ancestors HDR [ROOT] | skill-pin siblings HDR [ROOT] | skill-pin cousins HDR [ROOT] | skill-pin uncles HDR [ROOT] | skill-pin nephews HDR [ROOT] | skill-pin grandchildren HDR [ROOT] | skill-pin great-grandchildren HDR [ROOT] | skill-pin great-great-grandchildren HDR [ROOT]",
            file=sys.stderr,
        )
        return 2
    print(
