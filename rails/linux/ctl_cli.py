...
def run(argv: list[str], ns: dict) -> int:
    ...
    if cmd == "skill-pin":
        ...
        if sub == "last":
            ...
            return 0 if report["ok"] else 1
        print(
            "usage: juniorctl skill-pin list [ROOT] | skill-pin load REL [ROOT] | skill-pin pin REL [ROOT] | skill-pin verify [ROOT] | skill-pin verify-one REL [ROOT] | skill-pin tip [ROOT] | skill-pin log [ROOT] | skill-pin height [ROOT] | skill-pin get HEIGHT [ROOT] | skill-pin at HDR [ROOT] | skill-pin range FROM TO [ROOT] | skill-pin since HDR [ROOT] | skill-pin until HDR [ROOT] | skill-pin before HDR [ROOT] | skill-pin after HDR [ROOT] | skill-pin first [ROOT] | skill-pin last [ROOT]",
            file=sys.stderr,
        )
        return 2
    print(
