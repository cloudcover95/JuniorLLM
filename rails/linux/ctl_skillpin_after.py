"""T20 — skill-pin after HDR. Loopback only. Never fetch. Never exec."""
...
def skill_pin_after(hdr: str | None = None, root: str | None = None) -> dict:
    """T20 — list SKILL_PINS.jsonl rows strictly after HDR through tip. Never body. Never fetch. Never exec."""
    ...
    return {
        "ok": chain_ok and found,
        "issues": uniq,
        "bind": "127.0.0.1:8765",
        "root": str(Path(raw).resolve()),
        "op": "after",
        "tip": pins.tip(),
        "hdr": first.hdr if first else want,
        "height": tip_row.height if window and tip_row is not None else -1,
        "from_hdr": want,
        "from_height": first.height if first is not None else -1,
        "count": len(window),
        "total": len(rows),
