"""T15 — skill-pin at HDR. Loopback only. Never fetch. Never exec."""
...
def skill_pin_at(hdr: str | None = None, root: str | None = None) -> dict:
    """T15 — get one SKILL_PINS.jsonl row by HDR. Never body. Never fetch. Never exec."""
    ...
    hit = next((row for row in rows if row.hdr == want), None)
    if hit is None:
