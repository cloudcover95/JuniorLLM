"""T35 — skill-pin grandchildren. Loopback only. Never fetch. Never exec."""
...
def _grandchildren(rows: list, root_hdr: str) -> tuple[list, list[str]]:
    ...
    peers = [row for row in rows if row.prev in kid_hdrs]
    peers.sort(key=lambda row: row.height)
    return peers, []


def skill_pin_grandchildren(hdr: str | None = None, root: str | None = None) -> dict:
    """T35 — list grandchild SKILL_PINS.jsonl rows of HDR. Never body. Never fetch. Never exec."""
    ...
    if elder is None:
        ...
        peers, walk_issues = _grandchildren(rows, want)
    ...
    return {
        ...
        "grandparent_hdr": want,
        "grandparent_height": elder.height if elder is not None else -1,
        "grandparent_op": elder.op if elder else "",
