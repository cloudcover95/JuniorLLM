"""T34 — skill-pin nephews. Loopback only. Never fetch. Never exec."""
...
def _nephews(rows: list, child_hdr: str) -> tuple[list, list[str]]:
    by_hdr = {row.hdr: row for row in rows}
    kid = by_hdr.get(child_hdr)
    if kid is None:
        return [], ["missing_hdr"]
    siblings = [row for row in rows if row.prev == kid.prev and row.hdr != kid.hdr]
    sib_hdrs = {row.hdr for row in siblings}
    peers = [row for row in rows if row.prev in sib_hdrs]
    peers.sort(key=lambda row: row.height)
    return peers, []


def skill_pin_nephews(hdr: str | None = None, root: str | None = None) -> dict:
    """T34 — list nephew SKILL_PINS.jsonl rows of HDR. Never body. Never fetch. Never exec."""
    ...
    if child is None:
        ...
        peers, walk_issues = _nephews(rows, want)
