"""T31 — skill-pin siblings. Loopback only. Never fetch. Never exec."""
...
def _siblings(rows: list, child_hdr: str) -> tuple[list, list[str]]:
    by_hdr = {row.hdr: row for row in rows}
    kid = by_hdr.get(child_hdr)
    if kid is None:
        return [], ["missing_hdr"]
    peers = [row for row in rows if row.prev == kid.prev and row.hdr != kid.hdr]
    peers.sort(key=lambda row: row.height)
    return peers, []


def skill_pin_siblings(hdr: str | None = None, root: str | None = None) -> dict:
    """T31 — list sibling SKILL_PINS.jsonl rows of HDR. Never body. Never fetch. Never exec."""
    ...
    if child is None:
        ...
        peers, walk_issues = _siblings(rows, want)
