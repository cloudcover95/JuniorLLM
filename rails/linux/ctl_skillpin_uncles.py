"""T33 — skill-pin uncles. Loopback only. Never fetch. Never exec."""
...
def _uncles(rows: list, child_hdr: str, zero: str) -> tuple[list, list[str]]:
    ...
    peers = [row for row in rows if row.prev == parent.prev and row.hdr != parent.hdr]
    peers.sort(key=lambda row: row.height)
    return peers, []


def skill_pin_uncles(hdr: str | None = None, root: str | None = None) -> dict:
    """T33 — list uncle SKILL_PINS.jsonl rows of HDR. Never body. Never fetch. Never exec."""
    ...
    if child is None:
        ...
        peers, walk_issues = _uncles(rows, want, ZERO)
