"""T10 — juniorctl skill-pin verify-one REL is loopback-only and never fetches."""
...
class T10JuniorctlSkillPinVerifyOneTests(unittest.TestCase):
    ...
    def test_verify_one_unpinned_then_pinned_then_mismatch(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            rel = self._tree(td)
            report = juniorctl.skill_pin_verify_one(rel, str(td))
            self.assertTrue(report["ok"], report)
            self.assertEqual(report["bind"], "127.0.0.1:8765")
            self.assertEqual(report["op"], "verify-one")
    ...
    def test_verify_one_missing_after_pin(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            rel = self._tree(td)
            pinned = juniorctl.skill_pin_pin(rel, str(td))
            ...
            report = juniorctl.skill_pin_verify_one(rel, str(td))
            self.assertFalse(report["ok"])
            self.assertEqual(report["issues"], ["missing"])
            self.assertEqual(report["skills"][0]["state"], "missing")

    def test_cli_skill_pin_verify_one_exit_zero(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            rel = self._tree(td)
            ...
            with redirect_stdout(buf):
                code = juniorctl.main(
                    ["juniorctl", "skill-pin", "verify-one", rel, str(td)]
                )
            self.assertEqual(code, 0)
            payload = json.loads(buf.getvalue())
            self.assertTrue(payload["ok"])
            self.assertEqual(payload["bind"], "127.0.0.1:8765")
            self.assertEqual(payload["op"], "verify-one")
            self.assertEqual(payload["rel"], rel)
            self.assertEqual(payload["matched"], 1)

    def test_cli_verify_one_mismatch_exits_one(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            rel = self._tree(td)
            juniorctl.skill_pin_pin(rel, str(td))
    ...
    def test_cli_verify_one_missing_rel_is_denied(self):
        buf = io.StringIO()
        with redirect_stdout(buf):
            code = juniorctl.main(["juniorctl", "skill-pin", "verify-one"])
        self.assertEqual(code, 1)
        payload = json.loads(buf.getvalue())
        self.assertFalse(payload["ok"])
        self.assertEqual(payload["issues"], ["empty_path"])

    def test_refuse_abs_docker_wildcard_escape(self):
        sock = juniorctl.skill_pin_verify_one(
            "skills/x/SKILL.md", "/var/run/docker.sock"
        )
        self.assertFalse(sock["ok"])
        self.assertEqual(sock["issues"], ["denied_name"])
        wild = juniorctl.skill_pin_verify_one("skills/x/SKILL.md", "0.0.0.0")
        self.assertFalse(wild["ok"])
        self.assertEqual(wild["issues"], ["wildcard"])
        esc = juniorctl.skill_pin_verify_one("../escape/SKILL.md", "/tmp")
        self.assertFalse(esc["ok"])
        self.assertEqual(esc["issues"], ["path_escape"])
        pat = juniorctl.skill_pin_verify_one("skills/x/SKILL.md", "github_pat_secret")
        self.assertFalse(pat["ok"])
        abs_rel = juniorctl.skill_pin_verify_one("/etc/passwd", "/tmp")
        self.assertFalse(abs_rel["ok"])
        self.assertEqual(abs_rel["issues"], ["absolute_path"])

    def test_health_lists_skill_pin_verify_one(self):
        src = (ROOT / "rails" / "linux" / "juniorctl.py").read_text(encoding="utf-8")
        self.assertIn('"skill-pin verify-one"', src)
        self.assertIn("skill_pin_verify_one", src)
