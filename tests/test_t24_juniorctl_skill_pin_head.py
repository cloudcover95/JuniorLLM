"""T24 — juniorctl skill-pin head is loopback-only and never fetches."""
...
class T24JuniorctlSkillPinHeadTests(unittest.TestCase):
    ...
    def test_head_empty_then_pin_then_load(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            rel = self._tree(td)
            missing = juniorctl.skill_pin_head("8", str(td))
            self.assertTrue(missing["ok"], missing)
            self.assertEqual(missing["bind"], "127.0.0.1:8765")
            self.assertEqual(missing["op"], "head")
            ...
            only = juniorctl.skill_pin_head("2", str(td))
            ...
            still = juniorctl.skill_pin_head("2", str(td))
            ...
            one = juniorctl.skill_pin_head("1", str(td))
            first = juniorctl.skill_pin_first(str(td))
            self.assertTrue(one["ok"], one)
            self.assertEqual(one["count"], 1)
            self.assertEqual(one["n"], 1)
            self.assertEqual(one["hdr"], first["hdr"])
            self.assertEqual(one["hdr"], still["genesis"])
            self.assertEqual(one["last_op"], "pin")
            self.assertEqual(one["entries"][0]["op"], "pin")
            self.assertEqual(one["height"], 0)
            defaulted = juniorctl.skill_pin_head(None, str(td))
    ...
    def test_cli_skill_pin_head_exit_zero(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            rel = self._tree(td)
    ...
    def test_cli_head_bad_n(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            bad = juniorctl.skill_pin_head("tip", str(td))
            self.assertFalse(bad["ok"])
            self.assertEqual(bad["issues"], ["bad_n"])
            zero = juniorctl.skill_pin_head("0", str(td))
            self.assertFalse(zero["ok"])
            self.assertEqual(zero["issues"], ["bad_n"])
            neg = juniorctl.skill_pin_head("-1", str(td))
            self.assertFalse(neg["ok"])
            self.assertEqual(neg["issues"], ["bad_n"])
            buf = io.StringIO()
            with redirect_stdout(buf):
                code = juniorctl.main(
                    ["juniorctl", "skill-pin", "head", "tip", str(td)]
                )
    ...
    def test_refuse_docker_wildcard_escape(self):
        sock = juniorctl.skill_pin_head("2", "/var/run/docker.sock")
        self.assertFalse(sock["ok"])
        self.assertEqual(sock["issues"], ["denied_name"])
        wild = juniorctl.skill_pin_head("2", "0.0.0.0")
        self.assertFalse(wild["ok"])
        self.assertEqual(wild["issues"], ["wildcard"])
        esc = juniorctl.skill_pin_head("2", "../escape")
        self.assertFalse(esc["ok"])
        self.assertEqual(esc["issues"], ["path_escape"])
        pat = juniorctl.skill_pin_head("2", "github_pat_secret")
        self.assertFalse(pat["ok"])

    def test_health_lists_skill_pin_head(self):
        src = (ROOT / "rails" / "linux" / "juniorctl.py").read_text(encoding="utf-8")
        self.assertIn('"skill-pin head"', src)
        self.assertIn("skill_pin_head", src)
        ...
        self.assertIn("skill-pin head", health["cmds"])
