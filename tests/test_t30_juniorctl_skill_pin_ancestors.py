"""T30 — juniorctl skill-pin ancestors is loopback-only and never fetches."""
...
class T30JuniorctlSkillPinAncestorsTests(unittest.TestCase):
    ...
    def test_ancestors_empty_then_pin_then_load(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            rel = self._tree(td)
            missing = juniorctl.skill_pin_ancestors(ZERO, str(td))
            self.assertFalse(missing["ok"], missing)
            self.assertEqual(missing["bind"], "127.0.0.1:8765")
            self.assertEqual(missing["op"], "ancestors")
            ...
            of_gen = juniorctl.skill_pin_ancestors(tip0["hdr"], str(td))
            ...
            of_tip = juniorctl.skill_pin_ancestors(tip1["hdr"], str(td))
            ...
            self.assertEqual(len(of_tip["entries"]), 1)
            self.assertEqual(of_tip["entries"][0]["op"], "pin")
            self.assertEqual(of_tip["entries"][0]["height"], 0)
            self.assertEqual(of_tip["entries"][0]["hdr"], tip0["hdr"])
            self.assertNotIn("body", of_tip["entries"][0])
            still_gen = juniorctl.skill_pin_ancestors(tip0["hdr"], str(td))
            ...
            spine = juniorctl.skill_pin_ancestors(tip2["hdr"], str(td))
            self.assertTrue(spine["ok"], spine)
            self.assertEqual(spine["count"], 2)
            self.assertEqual(spine["total"], 3)
            self.assertEqual(spine["height"], 0)
            self.assertEqual(spine["last_op"], "pin")
            self.assertEqual(spine["hdr"], tip1["hdr"])
            self.assertEqual([row["height"] for row in spine["entries"]], [1, 0])
            self.assertEqual(spine["entries"][0]["hdr"], tip1["hdr"])
            self.assertEqual(spine["entries"][0]["op"], "load")
            self.assertEqual(spine["entries"][1]["hdr"], tip0["hdr"])
            self.assertEqual(spine["entries"][1]["op"], "pin")
            ...
            ghost = "a" * 64
            absent = juniorctl.skill_pin_ancestors(ghost, str(td))
    ...
    def test_cli_skill_pin_ancestors_exit_zero(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            rel = self._tree(td)
    ...
    def test_cli_ancestors_missing_and_bad_hdr(self):
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            buf = io.StringIO()
            with redirect_stdout(buf):
                code = juniorctl.main(
                    ["juniorctl", "skill-pin", "ancestors", ZERO, str(td)]
                )
            self.assertEqual(code, 1)
            payload = json.loads(buf.getvalue())
            self.assertFalse(payload["ok"])
            self.assertEqual(payload["issues"], ["missing_hdr"])
        empty = juniorctl.skill_pin_ancestors("", "/tmp")
        self.assertFalse(empty["ok"])
        self.assertEqual(empty["issues"], ["empty_hdr"])
        bad = juniorctl.skill_pin_ancestors("tip", "/tmp")
        self.assertFalse(bad["ok"])
        self.assertEqual(bad["issues"], ["bad_hdr"])
        short = juniorctl.skill_pin_ancestors("ab", "/tmp")
        self.assertFalse(short["ok"])
        self.assertEqual(short["issues"], ["bad_hdr"])
        heightish = juniorctl.skill_pin_ancestors("0", "/tmp")
        self.assertFalse(heightish["ok"])
        self.assertEqual(heightish["issues"], ["bad_hdr"])
        buf = io.StringIO()
        with redirect_stdout(buf):
            code = juniorctl.main(["juniorctl", "skill-pin", "ancestors"])
        self.assertEqual(code, 1)
        payload = json.loads(buf.getvalue())
        self.assertFalse(payload["ok"])
    ...
    def test_refuse_docker_wildcard_escape(self):
        sock = juniorctl.skill_pin_ancestors(ZERO, "/var/run/docker.sock")
        self.assertFalse(sock["ok"])
        self.assertEqual(sock["issues"], ["denied_name"])
        wild = juniorctl.skill_pin_ancestors(ZERO, "0.0.0.0")
        self.assertFalse(wild["ok"])
        self.assertEqual(wild["issues"], ["wildcard"])
        esc = juniorctl.skill_pin_ancestors(ZERO, "../escape")
        self.assertFalse(esc["ok"])
        self.assertEqual(esc["issues"], ["path_escape"])
        pat = juniorctl.skill_pin_ancestors(ZERO, "github_pat_secret")
        self.assertFalse(pat["ok"])

    def test_health_lists_skill_pin_ancestors(self):
        src = (ROOT / "rails" / "linux" / "juniorctl.py").read_text(encoding="utf-8")
        self.assertIn('"skill-pin ancestors"', src)
        self.assertIn("skill_pin_ancestors", src)
        ...
        self.assertIn("skill-pin ancestors", health["cmds"])
